<?php
	...
	if(isset($_POST["submit"])){
		...
		if (file_exists($target_file)) {
		    $resultadoSubida = "La imagen ya existe. ";
		    $uploadOk = 0;
		}
				...
				<div class="row">
					...
					<div class="col-md-5">
						<form class="form-inline" id="form1" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
							<input type="submit" value="Ganador" name="ganador">
						</form>
					</div>
					<div class="col-md-7">
						<p>
							<?php
								if(isset($_POST["ganador"])){
									if($count>0){
										$ganador = rand(1, $count);

										rewind($rifaHandle);
										for($i = 0; $i < $ganador; $i++){
											$line = fgets($rifaHandle);
										}
										$segment = explode(" ", $line);
										echo "participante #".$ganador.": folio = ".$segment[0].", monto = ".$segment[1].", correo = ".$segment[2].", telefono = ".$segment[3].", sucursal = ".$segment[4];
									}
								}
